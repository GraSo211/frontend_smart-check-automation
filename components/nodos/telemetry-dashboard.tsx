...
const COLORS = { normal: "var(--success)", warning: "var(--warning)", critical: "var(--critical)" }
...
function RamChart({ rows, total, used }: { rows: SpecificDevice[]; total?: number; used?: number }) { if (!total) return <div className="flex h-40 items-center justify-center text-sm text-muted-foreground">RAM total no disponible en este servidor.</div>; const free=rows.map(r=>percent(r.memRamDisponibleMb,total) ?? 0); return <div><svg viewBox="0 0 640 190" className="chart-svg" role="img" aria-label="Área apilada de RAM usada y libre"><Grid /><polygon fill="color-mix(in srgb, var(--series-memory) 20%, transparent)" points={`0,190 ${svgPoints(free.map(v=>100-v),640,190)} 640,190`} /><polyline fill="none" stroke="var(--series-memory)" strokeWidth="3" points={svgPoints(free,640,190)} /></svg><div className="mt-2 flex justify-between text-[11px] text-muted-foreground"><Legend color="var(--series-memory)" text={`${formatSnapshot(used, " MB")} usada`} /><Legend color="color-mix(in srgb, var(--series-memory) 40%, transparent)" text={`${formatSnapshot(total-(used ?? 0), " MB")} libre`} /></div></div> }
